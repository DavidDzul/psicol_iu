<template>
  <v-dialog :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)" max-width="700px" @keydown.stop.esc="close" :persistent="true">
    <v-card>
      <v-form>
        <v-card-title> Actualizar Autorización </v-card-title>
        <v-card-text>
          <v-row>
            <v-col cols="12" md="12">
...
import { Autorization, CauseEmun, StatusAutorizationEmun, UpdateAutorizationInput } from "@/grapqhl"
...
const { defineField, meta, values, resetForm, setValues } = useForm<UpdateAutorizationInput>({
...
const props = defineProps({
  ...
  editItem: { type: Object as PropType<Autorization>, required: false },
})

const emit = defineEmits<{
  "update:modelValue": [value: boolean]
  submit: [value: UpdateAutorizationInput]
}>()
    ...
    if (!value) {
      ...
      setTimeout(() => {
        setValues({
          ...
          numberMonths: value.numberMonths,
          previousMonths: value.previousMonths,
          cause: value.cause,
          otherCause: value.otherCause,
        })
      }, 1000)
    }
  },
